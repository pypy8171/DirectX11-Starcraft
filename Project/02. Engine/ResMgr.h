...
public:
	...
	void AddClone(CRes* _pRes){ m_vecCloneRes.push_back(_pRes);}

	void DeleteResource(RESOURCE_TYPE _eType, const wstring& _strKey);
...
inline T * CResMgr::Load(const wstring & _strKey, const wstring & _strPath)
{
	...
	if (!_strPath.empty())
	{
		...
		//return (T*)m_mapRes[(UINT)eType].find(_strKey)->second;
		return pRes;
	}
